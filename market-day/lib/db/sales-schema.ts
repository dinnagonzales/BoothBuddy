...
async function ensureAdditiveSalesColumns(db: SalesSchemaDb): Promise<void> {
  ...
  if (!salesColumns.includes('cancelled')) {
    await db.execAsync('ALTER TABLE sales ADD COLUMN cancelled INTEGER NOT NULL DEFAULT 0');
  }
  if (!salesColumns.includes('cancel_reason')) {
    await db.execAsync('ALTER TABLE sales ADD COLUMN cancel_reason TEXT');
  }
  if (!salesColumns.includes('cancel_note')) {
    await db.execAsync('ALTER TABLE sales ADD COLUMN cancel_note TEXT');
  }
}
