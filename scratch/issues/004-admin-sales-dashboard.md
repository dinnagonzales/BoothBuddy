...
The **Settings** tab (grown-up area, PIN-gated) shows a dashboard for the **Active Market Day** only: total revenue, **Item** count sold, breakdown by **Payment method** (Cash vs Venmo/Zelle), **Tips** (kept change), and **Sale** count. Below totals, a scrollable list of **Sales** for that day — each row shows **Sale number** (or optional **Sale name** when set), time, total, and payment method.
...
- [x] **Tips** total shown (kept change from cash sales)
